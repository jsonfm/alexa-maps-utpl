...
def extract_query_key_and_value(
    string: str, possible_keys: list, possible_values: list
):
...
class Database:
    ...
    def get_careers(self, randomized: bool = True, limit: int = 8):
        ...
        if self.df is None:
            return []
        careers = self.df["Carreras beneficiadas"].str.strip().dropna().unique()
    ...
    def single_query(
        ...
        try:
            ...
            data = (
                self.df.query(f"{comparation_key} == '{comparation_value}'")[
                    retrieve_key
                ]
                .dropna()
                .unique()
    ...
    def get_country_related_info(self, country: str):
        ...
        data = {"institutions": institutions, "covenios": convenios, "careers": careers}
        return data

    def parse_continent(self, continent: list):
        continents = {"Americano": "america", "Europa": "europe"}
        if len(continent) > 0:
...
class Database2:
    ...
    def load(self, file: str, questions: bool = True):
        """loads an excel file"""
        if ".xlsx" in file:
            self.df = pd.read_excel(file)
        if ".csv" in file:
    ...
    def load_questions(self):
        for row in self.df.iloc:
            question = f"{row['Tipo de pregunta']} {row['Palabra clave secundaria']} {row['Palabra principal']}"
...
db = Database()
dbv2 = Database2()
dbv2.load("db.csv", questions=True)
